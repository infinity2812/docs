...
When a borrower is matched with a lender, the interest rate they pay is determined by the P2P rate, which is derived from the current APYs of the selected optimizer’s underlying protocol.
...
Paystream’s P2P matching model allows for 100% capital utilization when a lender is matched. Unlike pool-based systems where idle capital often sits unused, Paystream ensures that every dollar matched P2P is actively generating yield.

In traditional systems:
- Lenders often deposit into a pool and wait for borrowers.
- Borrowers may face delays or partial fills due to pool limits.
