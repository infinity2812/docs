...
Rebalancing is an optional feature that automatically adjusts token ratios and price points to reduce risk and optimize returns. Triggered by an off-chain system, it helps users avoid impermanent loss and liquidation without manual effort. A small fee applies based on the selected trigger type.
