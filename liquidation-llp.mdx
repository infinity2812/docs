...
To manage risk effectively, the protocol employs multiple layers of protection beyond full liquidation. Partial liquidation is triggered when an individual borrower’s LTV crosses a certain threshold, allowing only a portion of their position to be sold to bring their LTV back to a safer range. In extreme cases where overall pool utilization exceeds safe limits, the protocol initiates deleveraging, proportionally reducing all borrower debts to restore system-wide liquidity. Borrowers can also avoid liquidation altogether by increasing their margin, either manually by adding more collateral or (in the future) automatically through wallet-based top-ups.
...
It forces partial repayments by all borrowers proportionally, helping the protocol bring utilization back to a safe level.
