...
- Define two LTV thresholds:
    - `ltv_partial` – triggers **partial liquidation**
    - `ltv_full` – triggers **full liquidation**
- avoid liquidation - increase your margin
- **Trigger logic**:
    - If `current_LTV ≥ ltv_partial` but `< ltv_full`:
        
        → Calculate Δ (delta) using the target LTV equation and **partially liquidate** that amount.
        
    - If `current_LTV ≥ ltv_full`:
        
        → **Fully liquidate** the remaining position.
        
- **Partial liquidation**:
    - Happens **only once per cycle**, when the LTV first hits the partial threshold.
    - borrower can add more funds after one partial liquidation to be eligible for another partial liquidation
    - If no action is taken and the LTV worsens, **full liquidation** is triggered.
- **Deleveraging** may also occur independently to **free up lender funds** when system-wide utilization is high.
...
we deleverage that is reduce the levergae (lenders fund that the borrw
