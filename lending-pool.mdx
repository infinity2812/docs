...
### Lender Liquidity Index 

The **LLI** tracks cumulative yield earned by the lending pool over time. Each time a borrower accrues interest, the LLI updates to reflect that yield.
...
```
lenderBalance = deposit × (LLI_now / LLI_entry)
```
...
```
LLI(t) = LLI(t-1) × (1 + liquidityRate × Δt)
```
