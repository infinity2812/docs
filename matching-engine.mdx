...
Whenever the P2P queue can’t immediately pair lenders with borrowers, PayStream automatically routes the idle liquidity into the chosen underlying protocol—such as Drift, Kamino, MarginFi, or Save Finance—so the funds keep earning the base supply APY until a matching borrower arrives.
